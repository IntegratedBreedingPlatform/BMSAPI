...
public class UserResourceBrapiTest extends ApiUnitTestBase {
	...
	public void testListUsers() throws Exception {
		...
		this.mockMvc.perform(MockMvcRequestBuilders.get(uriComponents.toUriString()).contentType(this.contentType))
				...
				.andExpect(MockMvcResultMatchers.jsonPath("$[0].userId", Matchers.is(users.get(0).getUserId())))
	...
	public void testCreateUser() throws Exception {
		final UserDetailsDto user = inicializeUser();
		final GenericResponse response = new GenericResponse("10");
		final UriComponents uriComponents = UriComponentsBuilder.newInstance().path("/brapi/v1/users").build().encode();

		Mockito.when(this.userService.createUser(Mockito.any(org.ibp.api.brapi.v1.user.UserDetailsDto.class))).thenReturn(response);
	...
	public void testUpdateUser() throws Exception {
		...
		Mockito.when(this.userService.updateUser(Mockito.any(org.ibp.api.brapi.v1.user.UserDetailsDto.class))).thenReturn(response);
	 ...
	 */
	public UserDetailsDto inicializeUser() {
	...
	/**
	 * Inicialize List UserDetailsDto
