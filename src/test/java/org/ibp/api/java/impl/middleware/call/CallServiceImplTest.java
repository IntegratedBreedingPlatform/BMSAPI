...
public class CallServiceImplTest {
	...
	public void testGetAllCalls() {
		...
		Assert.assertEquals("Second page should contain 8 records", 8, result2.size());

		final List<Map<String, Object>> result3 = this.callService.getAllCalls(null, null, null);
		Assert.assertEquals("Should return all records if pageSize and pageNumber are not specified", 18, result3.size());
