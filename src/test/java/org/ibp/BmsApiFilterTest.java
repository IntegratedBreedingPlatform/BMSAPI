...
public class BmsApiFilterTest {
	...
	public void testHeaders() throws Exception {
		...
		Mockito.verify(servletResponseMock).setHeader("Access-Control-Allow-Methods", "POST, PUT, PATCH, GET, OPTIONS, DELETE");
		Mockito.verify(servletResponseMock).setHeader("Access-Control-Max-Age", "3600");
		Mockito.verify(servletResponseMock).setHeader("Access-Control-Allow-Headers", "x-requested-with, x-auth-token, Authorization, Content-type");
