...
public class ScaleSummarySerializer extends JsonSerializer<ScaleSummary> {
	...
	public void serialize(ScaleSummary scaleSummary, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException {
		...
		if (!this.isNullOrEmpty(scaleSummary.getId())) {
		...
		if (scaleSummary.getDataType() != null) {
			...
			if (this.modelService.isNumericDataType(scaleSummary.getDataType().getId())) {
				if (!this.isNullOrEmpty(scaleSummary.getValidValues().getMin())) {
					BigDecimal min = new BigDecimal(scaleSummary.getValidValues().getMin());
					jsonGenerator.writeNumberField("min", min);
				}

				if (!this.isNullOrEmpty(scaleSummary.getValidValues().getMax())) {
			...
			} else if (this.modelService.isCategoricalDataType(scaleSummary.getDataType().getId())
					&& !scaleSummary.getValidValues().getCategories().isEmpty()) {
