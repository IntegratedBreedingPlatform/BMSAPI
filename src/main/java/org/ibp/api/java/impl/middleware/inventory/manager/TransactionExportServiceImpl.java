...
public class TransactionExportServiceImpl implements TransactionExportService {
	...
	private static final int TRANSACTIONS_SHEET_STORAGE_LOCATION_ABBR_COLUMN_INDEX = 4;
	private static final int TRANSACTIONS_SHEET_STORAGE_LOCATION_COLUMN_INDEX = 5;
	private static final int TRANSACTIONS_SHEET_STOCK_ID_COLUMN_INDEX = 6;
	...
	private void writeTransactionsHeader(final HSSFWorkbook xlsBook, final HSSFSheet xlsSheet, final int currentRowNum) {
		...
		cell = row.createCell(TransactionExportServiceImpl.TRANSACTIONS_SHEET_CROSS_COLUMN_INDEX, CellType.STRING);
		cell.setCellStyle(this.getHeaderStyle(xlsBook, IndexedColors.YELLOW.getIndex()));
		cell.setCellValue(this.messageSource.getMessage("export.inventory.manager.transaction.template.sheet.cross.column", null, locale));
