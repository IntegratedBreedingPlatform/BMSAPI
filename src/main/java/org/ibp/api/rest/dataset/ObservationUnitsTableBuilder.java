...
public class ObservationUnitsTableBuilder {
	...
	public static Table buildObservationUnitsTable (final List<List<String>> data, List<MeasurementVariableDto> datasetMeasurementVariables, List<String> warnings) {
		...
		for (final List<String> row: values) {
			...
			if (!table.containsRow(row.get(obsUnitIdIndex))) {
				...
				warnings.add("Some data in the import sheet is duplicated and only the first occurrence will be stored. Would you like to proceed with the import?");
			}
		}
