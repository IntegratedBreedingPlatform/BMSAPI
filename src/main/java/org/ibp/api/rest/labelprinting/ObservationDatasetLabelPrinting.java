...
public class ObservationDatasetLabelPrinting extends LabelPrintingStrategy {
		...
		for (final ObservationUnitRow observationUnitRow : observationUnitRows) {
			...
			for (final Integer requiredField : allRequiredKeys) {
				...
				if (!STATIC_FIELD_IDS.contains(field.getId())) {
					...
					final String ObsUnitId = observationUnitRow.getObsUnitId();
	...
	private void getInventoryDataRow(
		...
		if (ObservationDatasetLabelPrinting.LOT_FIELD.getById(id) != null) {
			switch (ObservationDatasetLabelPrinting.LOT_FIELD.getById(id)) {
				case LOT_UID:
		...
		else if (ObservationDatasetLabelPrinting.TRANSACTION_FIELD.getById(id) != null) {
			switch (ObservationDatasetLabelPrinting.TRANSACTION_FIELD.getById(id)) {
				case TRN_ID:
